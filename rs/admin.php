...
--Datalayer
--handles datalayer manipulations for ADMIN users
...
--runs query if its permitted (which, for admins, is basically any)
...
*/
	...
	if(isset($_POST['input'])) //if input was passed in
	{
		...
		{
			if(setData($input)==true) { echo 'Other query run'; }//run query that changes data
			else { echo 'Warning:'.$conn->error;  }	//if query failed, return warning
		}
