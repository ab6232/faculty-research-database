...
--Datalayer
--handles datalayer manipulations for authentication of ADMINS
...
--compares the credentials against the Admins table in the database
--Returns: true if that pair of username and password exist in the Admins database, false if not
*/
	...
	function execute($conn,$username,$password){
		...
		while($row = $result->fetch_assoc()){
			...
			if($row["username"]==$username && $row["password"]==$password){	//if username and password are in the table
				$isFound = "true";
			}
		}
